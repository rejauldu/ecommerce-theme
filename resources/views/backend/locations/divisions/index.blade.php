						...
						<table id="dataTables" class="display nowrap" cellspacing="0" width="100%">
							...
							<tbody>
								...
								<tr>
									<td>{{ $division->id }}</td>
									<td>{{ $division->name }}</td>
									<td><a href="{{ route('divisions.edit', $division->id) }}" class="btn btn-success fa fa-edit"></a></td>
									<td><a href="{{ route('divisions.destroy', $division->id) }}" onclick="event.preventDefault(); document.getElementById('delete-form').action = this.href; document.getElementById('delete-form').submit.click();" class="btn btn-danger fa fa-trash"></button></td>
								</tr>
								@endforeach
							</tbody>
							<tfoot>
